...
def login(user, passwd):
    header = {
        ...
        "host": "sv.dut.udn.vn",
        "origin": "https://sv.dut.udn.vn",
        "referer": "https://sv.dut.udn.vn/PageDangNhap.aspx",
        "priority": "u=0, i",
    ...
    if status == None:
        return {
            ...
            "name": soup.find("span", {"id": "Main_lblHoTen"}).get_text(),
        }
    else:
        return {"status": "error", "reason": "authentication"}


def getCal(datecode, cookie):
    s = requests.Session()
    res = s.get(
        f"https://sv.dut.udn.vn/WebAjax/evLopHP_Load.aspx?E=TTKBLoad&Code={datecode}",
        cookies=cookie,
    )
    s.close()
    return res.text
